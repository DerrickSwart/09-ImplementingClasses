...
class Point(object):
    ...
    def move_to(self, x_new, y_new):
        x_dist = (x_new - self.x)
        y_dist = (y_new - self.y)
        self.traveled = self.traveled + math.sqrt((x_dist**2)+ (y_dist**2))
        self.x = x_new
        self.y = y_new
    ...
    def get_distance_from_start(self):
        dx = self.x - self.first_x
        dy = self.y - self.first_y
    ...
    def closer_to(self, p1, p2):
        point_p1 = math.sqrt(((self.x - p1.x)**2) + ((self.y - p1.y)**2))
        point_p2 = math.sqrt(((self.x - p2.x)**2) + ((self.y - p2.y)**2))
    ...
    def halfway_to(self, p2):
        half_x = (self.x + p2.x)/2
        half_y = (self.y + p2.y)/2
